...
static uint64_t	nftl_part_offset = 0;
...
static int aml_nand_add_partition(struct aml_nand_chip *aml_chip)
{
	...
	if (!memcmp(temp_parts->name, "NFTL_Part", 9)) {
		temp_parts = &parts[nr-2];
		nftl_part_offset = temp_parts->offset + temp_parts->size;
	}
	else
		nftl_part_offset = mtd->size;
